<x-app-layout>
    <div class="card card-default">
        <div class="card-header">
            <h3 class="card-title">{{($userInfo) ? 'Update' : 'Create'}} Usesr</h3>
        </div>
        ...
        <form method="POST">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <x-input type="text" name="name" id="name" class="form-control" value="{!! ($userInfo) ? $userInfo->title : old('name') !!}"/>
                </div>

                <div class="form-group">
                    <label for="title">Email</label>
                    @if($errors->has('email')) <div class="float-right">{{$errors->first('email')}}</div> @endif
                    <x-input type="email" name="email" id="email" class="form-control" value="{!! ($userInfo) ? $userInfo->title : old('email') !!}"/>
                </div>
            ...
            <div class="card-footer">
                <button type="submit" class="btn btn-primary">{{($userInfo) ? 'Update' : 'Create'}} role</button>
            </div>
        </form>
    </div>
